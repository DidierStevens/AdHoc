...
def SearchRepeatingString(line, oOutput, options):
    currentMaxRepetitions = 2
    currentMaxStringLength = 1
    for i1 in range(int(len(line) / 2)):
        for i2 in range(int(len(line) / 2), i1, -1):
            substring = line[i1:i2]
            result = FindAll(line, substring)
            if len(result) > currentMaxRepetitions and len(substring) > 1 or len(result) == currentMaxRepetitions and len(substring) > currentMaxStringLength:
                decoded = line.replace(substring, '')
                if options.find == '':
                    oOutput.Line('Repetitions: %d' % (len(result)))
                    oOutput.Line('Repeating string: %s' % (substring))
                    oOutput.Line('Decoded (first 40 bytes): %s' % decoded[:40])
                    oOutput.Line('')
                    currentMaxRepetitions = len(result)
                    currentMaxStringLength = len(substring)
                elif options.find in decoded:
    ...
    with TextFile(filename, oLogfile) as fIn:
        try:
            for line in ProcessFile(fIn, oBeginGrep, oGrep, oEndGrep, context, options, False):
                ...
                SearchRepeatingString(line, oOutput, options)
                # ----------------------------------------------
        except:
...
https://DidierStevens.com'''
    ...
    oParser.add_option('-f', '--find', type=str, default='', help='String to search for')
